...
def build_production_spec(plan):
    task=build_mpt_task(plan); raw=json.dumps(task,ensure_ascii=False).encode();
    return {'provider':'github','task_type':'video_batch','workflow':'render-short01.yml','branch':'main','content_id':plan.content_id,'hook':plan.hook,'script':plan.script,'cta':plan.cta,'title':plan.title,'description':plan.description,'artifact_name':f'remote-pay-guide-{plan.content_id}','task_payload':task,'task_payload_b64':base64.b64encode(raw).decode(),'template':'short_video_template','voice':task['voice_name'],'visual_direction':plan.visual_direction}
