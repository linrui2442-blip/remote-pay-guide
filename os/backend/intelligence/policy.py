...
def _conn():
    ...
    c.execute('CREATE UNIQUE INDEX IF NOT EXISTS uq_policy_current ON intelligence_policy_decisions(content_plan_id, content_plan_revision, policy_version) WHERE superseded_at IS NULL')
    c.commit(); return c
...
def evaluate_policy_signals(plan_record, signals):
    ...
    for key in REQUIRED_GATES:
        if gates[key]=='BLOCK': reasons.append({'duplicate_risk':'DUPLICATE_BLOCK','account_health':'ACCOUNT_HARD_FAILURE','platform_health':'PLATFORM_HARD_FAILURE'}.get(key,key.upper()+'_BLOCK'))
            ...
            reasons.append({'duplicate_risk':'DUPLICATE_RISK_UNKNOWN','account_health':'ACCOUNT_HEALTH_UNKNOWN','platform_health':'PLATFORM_HEALTH_UNKNOWN','frequency':'FREQUENCY_POLICY_UNKNOWN','business':'BUSINESS_POLICY_UNKNOWN','quality':'QUALITY_POLICY_UNKNOWN','cost':'COST_POLICY_UNKNOWN','ai_confidence':'AI_CONFIDENCE_UNKNOWN'}.get(key,key.upper()+'_UNKNOWN'))
    if any(gates[k]=='BLOCK' for k in REQUIRED_GATES) or any(x in reasons for x in ('PLAN_NOT_ACTIONABLE','CONTENT_SAFETY_BLOCK','NOVELTY_BLOCK')): decision='BLOCK'
    elif any(gates[k] in {'UNKNOWN','WARN'} for k in REQUIRED_GATES) or plan_record.get('status')=='approved': decision='REVIEW'
    else: decision='AUTO'; reasons=['AUTO_ELIGIBLE']
    return {'decision':decision,'reason_codes':list(dict.fromkeys(reasons)),'evidence':{'gates':gates,'revision':plan_record['revision']}}
...
def evaluate_policy(plan_id, signals=None):
    ...
    with _conn() as c:
        ...
        c.execute('UPDATE intelligence_policy_decisions SET superseded_at=? WHERE content_plan_id=? AND content_plan_revision=? AND policy_version=? AND superseded_at IS NULL',(_now(),plan_id,p['revision'],POLICY_VERSION))
...
def get_current_policy_decision(plan_id):
    p=get_plan(plan_id)
    if not p: raise KeyError('plan not found')
    with _conn() as c: row=c.execute('SELECT * FROM intelligence_policy_decisions WHERE content_plan_id=? AND superseded_at IS NULL ORDER BY id DESC LIMIT 1',(plan_id,)).fetchone()
    d=_row(row)
...
def list_review_queue():
    with _conn() as c: rows=c.execute('SELECT * FROM intelligence_policy_decisions WHERE decision="REVIEW" AND superseded_at IS NULL ORDER BY id').fetchall()
    out=[]
    for r in rows:
