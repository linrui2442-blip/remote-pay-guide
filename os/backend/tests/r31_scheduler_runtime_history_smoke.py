...
os.chdir(ROOT)
_bootstrap_root = Path(tempfile.mkdtemp(prefix="rpg-runtime-history-bootstrap-"))
os.environ["OS_TESTING"] = "1"
os.environ["OS_DATABASE_PATH"] = str(_bootstrap_root / "os.db")
...
def _configure(db_path):
    os.environ["OS_TESTING"] = "1"
    os.environ["OS_DATABASE_PATH"] = str(Path(db_path).resolve())
